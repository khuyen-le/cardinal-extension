...
save <- aictab(cand.set = list(fit.base_set_1, fit.cp_age_set_1), 
       modnames = c('base', 'cp_age'), sort = F)
bictab(cand.set = list(fit.base_set_1, fit.cp_age_set_1, fit.cp_age_int_set_1), 
       modnames = c('base', 'cp_age', 'cp_age_int'))
...
aictab(cand.set = list(fit.base_set_1, fit.cp_age_set_1, fit.cp_age_int_set_1), 
       modnames = c('base', 'cp_age', 'cp_age_int'))
bictab(cand.set = list(fit.base_set_1, fit.cp_age_set_1, fit.cp_age_int_set_1), 
       modnames = c('base', 'cp_age', 'cp_age_int'))
```
...
aictab(cand.set = list(fit.base_count_1, fit.cp_age_count_1, fit.cp_age_int_count_1), 
       modnames = c('base', 'cp_age', 'cp_age_int'))
bictab(cand.set = list(fit.base_count_1, fit.cp_age_count_1, fit.cp_age_int_count_1), 
       modnames = c('base', 'cp_age', 'cp_age_int'))
```
...
```{r}
df.trial_2 %>% filter(is.na(set_chosen), !is.na(final_count)) %>%
  pull(id) %>%
  unique()
df.trial_2 %>% filter(trial_type == "small", is.na(set_chosen), !is.na(final_count)) %>%
  pull(id) %>%
  unique()

df.trial_2 %>% filter(trial_type == "large-DR", is.na(set_chosen), !is.na(final_count)) %>%
  pull(id) %>%
  unique()

df.trial_2 %>% filter(trial_type == "large-NR", is.na(set_chosen), !is.na(final_count)) %>%
  pull(id) %>%
  unique()
```
...
t_largenr_2 <- t.test(df.trial_2 %>% 
               filter(set_chosen == "top" | set_chosen == "bottom") %>%          
         filter(trial_type == "large-NR") %>% 
         pull(correct_set_chosen), 
       mu = 0.5)
```

```{r study2 - variability plot}
...
df.summary_2_total <- df.trial_2 %>%
  group_by(id) %>%
  summarise(mean_correct_set_chosen = mean(correct_set_chosen)) %>%
  filter(mean_correct_set_chosen >= 8/9)

df.summary_2_total <- df.trial_2 %>%
  group_by(id) %>%
  summarise(mean_correct_set_chosen = mean(correct_set_chosen)) %>%
  filter(mean_correct_set_chosen < 1/2)
...
df.ppt_correct_nr_2 <- df.ppt_2 %>%
  filter(id %in% ppt_correct_nr) %>%
  summarise(mean_age = mean(age_years_cont), 
            mean_hc = mean(highest_count, na.rm = T))

df.ppt_not_correct_nr_2 <- df.ppt_2 %>%
  filter(!id %in% ppt_correct_nr) %>%
  summarise(mean_age = mean(age_years_cont), 
            mean_hc = mean(highest_count, na.rm = T))
```
...
aictab(cand.set = list(fit.base_set_2, fit.trial_type_set_2), 
       modnames = c('base', 'trial_type'))
bictab(cand.set = list(fit.base_set_2, fit.trial_type_set_2), 
       modnames = c('base', 'trial_type'))
...
df.trial_cp_count_wrong_2 %>%
  pull(id) %>%
  unique()

df.trial_cp_count_wrong_2 %>% filter(correct_set_chosen == 1) %>%
  pull(id) %>%
  unique()

df.trial_cp_count_wrong_2 %>% filter(correct_set_chosen == 0, !is.na(set_chosen)) %>%
  pull(id) %>%
  unique()

df.trial_cp_count_wrong_2 %>% filter(is.na(set_chosen)) %>%
  pull(id) %>%
  unique()
```
...
aictab(cand.set = list(fit.base_count_2, fit.trial_type_count_2), 
       modnames = c('base', 'trial_type'))
bictab(cand.set = list(fit.base_count_2, fit.trial_type_count_2), 
       modnames = c('base', 'trial_type'))
...
aictab(cand.set = list(fit.trial_type_set_2_hc_comp, fit.trial_type_set_2_hc), 
       modnames = c('trial_type_set_hc_comp', 'trial_type_set_hc'))
bictab(cand.set = list(fit.trial_type_set_2_hc_comp, fit.trial_type_set_2_hc), 
       modnames = c('trial_type_set_hc_comp', 'trial_type_set_hc'))
```
...
aictab(cand.set = list(fit.trial_type_count_2_hc_comp, fit.trial_type_count_2_hc), 
       modnames = c('trial_type_count_hc_comp', 'trial_type_count_hc'))
bictab(cand.set = list(fit.trial_type_count_2_hc_comp, fit.trial_type_count_2_hc), 
       modnames = c('trial_type_count_hc_comp', 'trial_type_count_hc'))
```

...
```{r study1 - highest count variability plot, fig.cap = "Histogram of participants' Highest Count by Knower Level.", fig.height = 4, include = T}
ggplot(df.ppt_1 %>%
         filter(!is.na(highest_count)) %>% 
         mutate(knower_level_cp_subset = factor(knower_level_cp_subset, levels = c("subset", "CP"))), 
       aes(x = highest_count, fill = knower_level_cp_subset, 
           color = knower_level_cp_subset)) + 
...
```{r exp1 supplemental, fig.width = 8, fig.height = 12}
ggplot(df.trial_1 %>%
         filter(!is.na(highest_count)) %>%
         group_by(id, knower_level_cp_subset, highest_count) %>%
         summarise(sum_correct_set_chosen = sum(correct_set_chosen)), 
...
```{r exp2-plot-var-old, fig.cap = "Variability in Experiment 2. All trials: A) Number of participants against proportion of correct item set choice. B) Number of participants against proportion of correct numerical response. By trial type: C) Number of participants against proportion of correct item set choice. D) Number of participants against proportion of correct numerical response.", fig.height = 7, include = T}
df.summary_2 <- df.trial_2 %>%
  group_by(id) %>%
  summarise(mean_correct_set_chosen = mean(correct_set_chosen),
            mean_correct_count_when_correct_set_chosen = mean(correct_count_when_correct_set_chosen))
...
```{r study2 - highest count variability plot, fig.cap = "Histogram of participants' Highest Count by Knower Level.", fig.height = 4, include = T}
...
```{r study2 - highest count variability plot OLD, fig.cap = "Histogram of participants' Highest Count by Knower Level.", fig.height = 4, include = T}
df.hc_2 <- df.ppt_2 %>%
  filter(!is.na(highest_count)) %>% 
  mutate(correct_nr = ifelse(id %in% ppt_correct_nr_2, "All correct", "At least 1 mistake"), 
         correct_nr = factor(correct_nr, levels = c("At least 1 mistake", "All correct")))

ggplot(df.hc_2, 
       aes(x = highest_count, fill = correct_nr, 
           color = correct_nr)) + 
  geom_histogram(position = 'identity', 
                 alpha = 0.5, 
                 binwidth = 1) + 
  ...
  labs(y = "Number of participants", 
       x = "Highest Count", 
       color = "Correct Set Choice\non off-by-one trials", 
       fill = "Correct Set Choice\non off-by-one trials")
...
```{r exp2 supplemental, fig.width = 8, fig.height = 8}
ggplot(df.trial_2 %>%
         filter(!is.na(highest_count)) %>%
         group_by(id, highest_count) %>%
         summarise(sum_correct_set_chosen = sum(correct_set_chosen)), 

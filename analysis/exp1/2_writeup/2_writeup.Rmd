...
authornote: |
  Part of these findings were presented as a poster at the 2024 Cognitive Development Society Biennial Meeting and as a talk at the 2024 Cognitive Science Society Meeting. The authors have no conflicts of interest to disclose. All materials, data, and analysis scripts are available at https://osf.io/eswa4 [@le2024]. This research was funded by a National Science Foundation award (#2000827) to DB. 
  
  \copyright American Psychological Association, 2024. This paper is not the copy of record and may not exactly replicate the authoritative document published in the APA journal. The final article is available, upon publication, at: 10.1037/dev0001922.
...
All materials, data, and analysis scripts for both Experiments 1 and 2 are available at https://osf.io/eswa4 [@le2024]. Data were analyzed using R [Version 4.1.3, @R-base] and the R-packages *lme4* [@R-lme4], *car* [@R-car], *emmeans* [@R-emmeans], *ggplot2* [@R-ggplot2], *cowplot* [@R-cowplot], *tidyverse* [@R-tidyverse], and *papaja* [@R-papaja]. The design and analysis of both experiments were preregistered, and we report how we determined our sample size, all data exclusions, all manipulations, and all measures in the study. A preregistration for Experiment 1 is available at https://osf.io/3v2cn. 
...
t_subset_1 <- t.test(df.trial_1 %>% 
         filter(knower_level_cp_subset == "subset") %>%
           group_by(id) %>%
           summarise(mean_correct_set = mean(correct_set_chosen)) %>% 
         pull(mean_correct_set),
       mu = 0.5)
...
```{r study1 set choice - subset with trials within KL - t tests}
t_subset_set_trials_within_kl_1 <- t.test(df.trial_1 %>% 
                       mutate(set_larger = substr(trial_ratio, 3, 3)) %>%
                       filter(knower_level_cp_subset == "subset") %>%
                       filter(knower_level >= set_larger) %>%
           group_by(id) %>%
           summarise(mean_correct_set = mean(correct_set_chosen)) %>% 
         pull(mean_correct_set),
       mu = 0.5)
...
```{r study1 set choice - CP-subset comparison - within KL}
t_small_within_kl_1 <- t.test(df.trial_1 %>%
                      filter(magnitude == "small", 
                             knower_level_cp_subset == "CP") %>%
           group_by(id) %>%
           summarise(mean_correct_set = mean(correct_set_chosen)) %>% 
...
t_set_within_kl_1 <- t.test(df.trial_1 %>%
                      filter(knower_level_cp_subset == "CP") %>%
           group_by(id) %>%
           summarise(mean_correct_set = mean(correct_set_chosen)) %>% 
         pull(mean_correct_set),
...
t_subset_count_1 <- t.test(df.trial_1 %>% 
         filter(knower_level_cp_subset == "subset") %>%
           group_by(id) %>%
           summarise(mean_correct_count = mean(correct_count_when_correct_set_chosen)) %>% 
         pull(mean_correct_count),
       mu = 0.5)
...
```{r study1 count - subset with trials within KL - t tests}
t_subset_count_trials_within_kl_1 <- t.test(df.trial_1 %>% 
                       mutate(set_larger = substr(trial_ratio, 3, 3)) %>%
                       filter(knower_level_cp_subset == "subset") %>%
                       filter(knower_level >= set_larger) %>%
           group_by(id) %>%
...
```{r study1 count - CP-subset comparison - within KL}
t_count_trials_within_kl_1 <- t.test(df.trial_1 %>%
                      filter(knower_level_cp_subset == "CP") %>%
           group_by(id) %>%
           summarise(mean_correct_count = mean(correct_count_when_correct_set_chosen)) %>% 
         pull(mean_correct_count), 
...
t_count_trials_within_kl_1 <- t.test(df.trial_1 %>%
         ...
         pull(mean_correct_count), 
                    df.trial_1 %>% 
                       mutate(set_smaller = substr(trial_ratio, 1, 1)) %>%
                       filter(knower_level_cp_subset == "subset") %>%
                       filter(knower_level >= set_smaller) %>%
           group_by(id) %>%
           summarise(mean_correct_count = mean(correct_count_when_correct_set_chosen)) %>% 
         pull(mean_correct_count)) 
```
...
apa_table(
  cbind(table, table2),
  caption = "Results of t-tests comparing cardinal extension performance between subset- and CP-knowers in Experiment 1.",
...
```{r exp1-plot-var, fig.cap = "A) Number of participants against proportion of correct set choice. B) Number of participants against proportion of correct numerical response.", fig.height = 3, include = T}
...
df.subset_summary_1 <- df.trial_1 %>%
  filter(knower_level_cp_subset == "subset") %>%
  group_by(id) %>%
  summarise(mean_correct_set_chosen = mean(correct_set_chosen))

df.summary_1 <- df.trial_1 %>%
  group_by(knower_level_cp_subset, id) %>%
  summarise(mean_correct_set_chosen = mean(correct_set_chosen),
            mean_correct_count_when_correct_set_chosen = mean(correct_count_when_correct_set_chosen))

plot_summary_set_1 <- ggplot(df.summary_1,
       aes(x = mean_correct_set_chosen,
           color = knower_level_cp_subset, 
           shape = knower_level_cp_subset)) +
...
plot_summary_count_1 <- ggplot(df.summary_1,
       aes(x = mean_correct_count_when_correct_set_chosen,
           color = knower_level_cp_subset,
           shape = knower_level_cp_subset)) +
  geom_line(stat = 'count') + 
  geom_point(stat = 'count',
  ...
  theme(text = element_text(size = 11)) + 
  labs(y = "Number of participants", 
       x = "Prop. of correct numerical response", 
       color = "Knower Level", 
       shape = "Knower Level")

combined_plot <- plot_grid(
  plot_summary_set_1 + theme(legend.position = "none"),
  plot_summary_count_1 + theme(legend.position = "none"), 
  labels = c("A", "B")
)
...
```{r study1 - regressions - set base - within KL, include = F}
fit.base_set_within_kl_1 <- glmer(correct_set_chosen ~ 
                          magnitude + age_zscored + (1|id) 
                        + (1|trial_ratio)
                        , 
                  data = df.trial_1 %>% 
                       mutate(set_larger = substr(trial_ratio, 3, 3)) %>%
                       filter((knower_level_cp_subset == "subset" & knower_level >= set_larger) | knower_level_cp_subset == "CP"), family="binomial")
summary(fit.base_set_within_kl_1)
fit.anova.base_set_within_kl_1 <- Anova(fit.base_set_within_kl_1, type = 3)
```

```{r study1 - regressions - set enhanced models - within KL, include = F}
fit.cp_age_set_within_kl_1 <- glmer(correct_set_chosen ~ knower_level_cp_subset + magnitude + age_zscored + (1|id) 
                          + (1|trial_ratio)
                          , data = df.trial_1 %>% 
                       mutate(set_larger = substr(trial_ratio, 3, 3)) %>%
                       filter((knower_level_cp_subset == "subset" & knower_level >= set_larger) | knower_level_cp_subset == "CP") 
                            , family="binomial")
summary(fit.cp_age_set_within_kl_1)
fit.anova.cp_age_set_within_kl_1 <- Anova(fit.cp_age_set_within_kl_1, type = 3)
fit.compare.cp_age_set_within_kl_1 <- anova(fit.cp_age_set_within_kl_1, fit.base_set_within_kl_1, type=3)

```

```{r exp1-plot, fig.cap = "Each dot represents a participant. The width of the shaded area of violin plots represents the proportion of the data located there. Error bars represent bootstrapped 95% CIs. A) Proportion of correct item set choice by participant against knower level. Horizontal dashed line indicates performance at chance = 0.50. B) Proportion of correct numerical response by participant against knower level.", fig.height = 4, include = T}
...
ppt_cp_count_mistake_1 <- df.cp_summary_count_1 %>%
...
`r df.trial_cp_count_wrong_1 %>% pull(id) %>% unique() %>% length() %>% printnum(., numerals = F, capitalize = T)` of the `r df.ppt_knower_level_1 %>% filter(knower_level_cp_subset == "CP") %>% pull(n)` CP-knowers (`r force_decimals(df.trial_cp_count_wrong_1 %>% pull(id) %>% unique() %>% length() / df.ppt_knower_level_1 %>% filter(knower_level_cp_subset == "CP") %>% pull(n) * 100)`%) gave a numerical response that was incorrect on at least one trial, resulting in a total of `r df.trial_cp_count_wrong_1 %>% nrow()` out of `r df.trial_1 %>% filter(knower_level_cp_subset == "CP") %>% nrow()` trials with an incorrect response. 
Some of these incorrect responses were due to procedural counting errors: on `r df.trial_cp_count_wrong_1 %>% filter(correct_set_chosen == 1) %>% nrow()` of these `r df.trial_cp_count_wrong_1 %>% nrow()` trials (`r df.trial_cp_count_wrong_1 %>% filter(correct_set_chosen == 1) %>% pull(id) %>% unique() %>% length()` CP-knowers) they counted the correct item set but made a mistake in their count (even though we gave them one chance to fix their wrong count). 
On `r df.trial_cp_count_wrong_1 %>% filter(correct_set_chosen == 0, !is.na(set_chosen)) %>% nrow()` trials (`r df.trial_cp_count_wrong_1 %>% filter(correct_set_chosen == 0, !is.na(set_chosen)) %>% pull(id) %>% unique() %>% length()` CP-knowers), they counted the wrong item set, or both item sets, which pointed to a lack of cardinal extension understanding. 
Finally, on the remaining `r df.trial_cp_count_wrong_1 %>% filter(is.na(set_chosen)) %>% nrow()` trials (`r df.trial_cp_count_wrong_1 %>% filter(is.na(set_chosen)) %>% pull(id) %>% unique() %>% length()` CP-knowers), CP-knowers provided the wrong numerical response without pointing to items in either visible set. This last behavior is compatible either with overconfidence in their ability to accurately estimate a set size, an attempt to count without pointing, or a failure to understand that the target number could be obtained by counting one of the visible sets. 
...
```{r study1 - regressions - count base - within KL, include = F}
fit.base_count_within_kl_1 <- glmer(correct_count_when_correct_set_chosen ~ magnitude + age_zscored + (1|id) + (1|trial_ratio), 
                          data = df.trial_1 %>% 
                       mutate(set_larger = substr(trial_ratio, 3, 3)) %>%
                       filter((knower_level_cp_subset == "subset" & knower_level >= set_larger) | knower_level_cp_subset == "CP"), family="binomial")
summary(fit.base_count_within_kl_1)
fit.anova.base_count_within_kl_1 <- Anova(fit.base_count_within_kl_1, type = 3)
```

```{r study1 - regressions - count enhanced models - within KL, include = F}
fit.cp_age_count_within_kl_1 <- glmer(correct_count_when_correct_set_chosen ~ knower_level_cp_subset + magnitude + age_zscored + (1|id) + (1|trial_ratio), 
                          data = df.trial_1 %>% 
                       mutate(set_larger = substr(trial_ratio, 3, 3)) %>%
                       filter((knower_level_cp_subset == "subset" & knower_level >= set_larger) | knower_level_cp_subset == "CP"), family="binomial")
summary(fit.cp_age_count_within_kl_1)
fit.anova.cp_age_count_within_kl_1 <- Anova(fit.cp_age_count_within_kl_1, type = 3)
fit.compare.cp_age_count_within_kl_1 <- anova(fit.cp_age_count_within_kl_1, fit.base_count_within_kl_1, type = 3)
```

To further explore the effect of CP-knower status on Correct Numerical Response, we constructed another set of GLMMs with the same fixed and random effect structure as the first analytical approach to predict children’s ability to infer the correct number of animals. Specifically, our base model had the formula Correct Numerical Response (0/1) ~ Age (z-scored) + Set Size (Small / Large) + (1 | Participant) + (1 | Trial), and we tested whether adding children’s Knower Level (CP / Subset) and Knower Level * Set Size interaction improved the fit of the model. The model that best explained the data included Age, Knower Level, Set Size, and Knower Level * Set Size interaction as predictors (compared against base model with only Age and Set Size: `r print_model_comp(fit.compare.cp_age_int_count_vs_base_1, "fit.cp_age_int_count_1")`, `r print_model_fit(fit.cp_age_int_count_1, fit.base_count_1)`; compared against model with Age, Set Size and Knower Level: `r print_model_comp(fit.compare.cp_age_int_count_vs_noint_1, "fit.cp_age_int_count_1")`, `r print_model_fit(fit.cp_age_int_count_1, fit.cp_age_count_1)`). CP-knowers were significantly better at determining the correct number of animals (`r print_coeff(fit.cp_age_int_count_1, "knower_level_cp_subsetCP", "$_{CP}$")`), as were older children (`r print_coeff(fit.cp_age_int_count_1, "age_zscored")`). We also found an effect of Set Size with children more likely to provide the correct numerical response in trials with small sets compared to those with large sets (`r print_coeff(fit.cp_age_int_count_1, "magnitudelarge", "$_{large}$")`)^[As suggested by an anonymous reviewer, we conducted a *post hoc* analysis with the same models, but removed any trials with sets beyond a particular subset-knower’s knower level. Results were qualitatively similar. In this case we did not conduct any analysis involving the Knower Level * Set Size interaction, because the subset-knowers would only contribute small trials and no large trials.]. 
Additionally, the Knower Level * Set Size interaction effect was significant, where subset-knowers showed a larger difference in performance between small and large trials compared to CP-knowers (`r print_coeff(fit.cp_age_int_count_1, "knower_level_cp_subsetCP:magnitudelarge", "$_{CP*large}$")`). These effects are illustrated in Figure \@ref(fig:exp1-plot)B.

```{r study 1 - highest count descriptives}
df.hc_summary_1 <- df.trial_1 %>%
  filter(!is.na(highest_count)) %>%
  group_by(knower_level_cp_subset) %>%
  summarise(mean_hc = mean(highest_count), 
            sd_hc = sd(highest_count), 
            min_hc = min(highest_count), 
            max_hc = max(highest_count))
```
...
check_collinearity(fit.cp_age_set_1_hc)
summary(fit.cp_age_set_1_hc)

fit.compare.cp_age_set_1_hc <- anova(fit.cp_age_set_1_hc, fit.cp_age_set_1_hc_comp, type=3)

aictab(cand.set = list(fit.cp_age_set_1_hc_comp, fit.cp_age_set_1_hc), 
       modnames = c('cp_age_hc_comp', 'cp_age_hc'))
bictab(cand.set = list(fit.cp_age_set_1_hc_comp, fit.cp_age_set_1_hc), 
       modnames = c('cp_age_hc_comp', 'cp_age_hc'))
```
...
check_collinearity(fit.cp_age_int_count_1_hc)
summary(fit.cp_age_int_count_1_hc)

fit.compare.cp_age_int_count_1_hc <- anova(fit.cp_age_int_count_1_hc, fit.cp_age_int_count_1_hc_comp, type=3)

aictab(cand.set = list(fit.cp_age_int_count_1_hc_comp, fit.cp_age_int_count_1_hc), 
       modnames = c('cp_age_int_hc_comp', 'cp_age_int_hc'))
bictab(cand.set = list(fit.cp_age_int_count_1_hc_comp, fit.cp_age_int_count_1_hc), 
       modnames = c('cp_age_int_hc_comp', 'cp_age_int_hc'))
```
...
fit.cp_age_set_1_hc_only_cp <- glmer(correct_set_chosen ~ highest_count + magnitude + age_zscored + (1|id) 
                          ...
                          %>% filter(!is.na(highest_count), 
                                     knower_level_cp_subset == "CP"), family="binomial")
check_collinearity(fit.cp_age_set_1_hc_only_cp)
...
fit.cp_age_int_count_1_hc_only_cp <- glmer(correct_count_when_correct_set_chosen ~ highest_count + magnitude + age_zscored + (1|id) + (1|trial_ratio), 
                          data = df.trial_1 %>%
                            filter(!is.na(highest_count), 
                                     knower_level_cp_subset == "CP"), family="binomial")
check_collinearity(fit.cp_age_int_count_1_hc_only_cp)
...
```{r exp1-hc-plot, fig.cap = "Participants’ average Highest Count by performance in the Correct Set Choice measure.", fig.height = 4, include = T}
ggplot(df.trial_1 %>%
         filter(!is.na(highest_count)) %>%
         group_by(id, knower_level_cp_subset, highest_count) %>%
         summarise(sum_correct_set_chosen = sum(correct_set_chosen)), 
       aes(x = sum_correct_set_chosen, 
           y = highest_count, 
           fill = knower_level_cp_subset)) +
  stat_summary(fun.y = "mean",
               geom = "col", 
               position = position_dodge2(width = 0.9, preserve = "single")) +
    stat_summary(aes(label= apa_num(round(..y..,2))),
  ...
  labs(y = "Average Highest Count", 
       x = "Number of trials with Correct Set Choice (out of 6)", 
       fill = "Knower Level")
```

As an exploratory analysis, we investigated whether children’s Highest Count explained further variability in our data, by adding it as a fixed effect to the best-performing models from the two analysis approaches above. `r df.ppt_1 %>% filter(!is.na(highest_count)) %>% pull(id) %>% length() %>% printnum(., numerals = F, capitalize = T)`children (out of `r df.ppt_1 %>% pull(id) %>% length()`) provided a Highest Count. As expected, CP-knowers ($M_{HC}$ = `r df.hc_summary_1 %>% filter(knower_level_cp_subset == "CP") %>% pull(mean_hc) %>% force_decimals()` [`r df.hc_summary_1 %>% filter(knower_level_cp_subset == "CP") %>% pull(min_hc)`; `r df.hc_summary_1 %>% filter(knower_level_cp_subset == "CP") %>% pull(max_hc)`]; $SD_{HC}$ = `r df.hc_summary_1 %>% filter(knower_level_cp_subset == "CP") %>% pull(sd_hc) %>% force_decimals()`) generally performed better in this task compared to subset-knowers  ($M_{HC}$ = `r df.hc_summary_1 %>% filter(knower_level_cp_subset == "subset") %>% pull(mean_hc) %>% force_decimals()` [`r df.hc_summary_1 %>% filter(knower_level_cp_subset == "subset") %>% pull(min_hc)`; `r df.hc_summary_1 %>% filter(knower_level_cp_subset == "subset") %>% pull(max_hc)`]; $SD_{HC}$ = `r df.hc_summary_1 %>% filter(knower_level_cp_subset == "subset") %>% pull(sd_hc) %>% force_decimals()`; for the distribution of Highest Count by Correct Set Choice performance, see Figure \@ref(fig:exp1-hc-plot) and Figure S1 in the supplemental material). After excluding the `r df.ppt_1 %>% filter(is.na(highest_count)) %>% pull(id) %>% length() %>% printnum(., numerals = F, capitalize = F)` children who did not provide a Highest Count (all subset-knowers), we found that adding Highest Count^[As noted by an anonymous reviewer, Highest Count might not be appropriate to use as a predictor in a GLMM. Children often count in runs, leading to certain Highest Count values being overly represented (e.g., the number before a new decade for English speakers). However, previous studies that include Highest Count as a predictor in regression models have found that it is predictive of other number knowledge [e.g., @chu2020; @davidson2012; @schneider2021]. Therefore, while this approach might not be perfect theoretically, we have reason to believe that we could use regression models to detect a relationship between Highest Count and Cardinal Extension understanding.] did not explain additional variability for either Correct Set (`r print_model_comp(fit.compare.cp_age_set_1_hc, "fit.cp_age_set_1_hc")`, `r print_model_fit(fit.cp_age_set_1_hc, fit.cp_age_set_1_hc_comp)`) or Correct Numerical Response (`r print_model_comp(fit.compare.cp_age_int_count_1_hc, "fit.cp_age_int_count_1_hc")`, `r print_model_fit(fit.cp_age_int_count_1_hc, fit.cp_age_int_count_1_hc_comp)`). When we limited this analysis to only CP-knowers, Highest Count also did not explain additional variation in the observed performance ($p$s > .05). In all models, Highest Count showed low multicollinearity with other predictors [Variance Inflation Factor < 2, following the threshold established by @james2013]^[We thank a reviewer for suggesting this *post hoc* analysis.].
...
All materials, data, and analysis code for the experiment are available at https://osf.io/eswa4 [@le2024]. A preregistration is available at https://osf.io/zrsw2. 
...
ppt_correct_nr_2 <- df.summary_2 %>%
  filter(`large-NR` == 1) %>%
  pull(id)

df.ppt_correct_nr_2 <- df.ppt_2 %>%
  filter(id %in% ppt_correct_nr_2) %>%
  summarise(mean_age = mean(age_years_cont), 
            mean_hc = mean(highest_count, na.rm = T))

df.ppt_not_all_correct_nr_2 <- df.ppt_2 %>%
  filter(!id %in% ppt_correct_nr_2) %>%
  summarise(mean_age = mean(age_years_cont), 
            mean_hc = mean(highest_count, na.rm = T))
```
...
However, this was only true for small-set trials ($M$ = `r df.summary_correct_set_chosen_2 %>% filter(trial_type == "small") %>% pull(mean) %>% force_decimals()`, $SD$ = `r df.summary_correct_set_chosen_2 %>% filter(trial_type == "small") %>% pull(sd) %>% force_decimals()`; `r print_ttest(t_small_2)`) and for large-set trials with perceptually-disciminable ratios ($M$ = `r df.summary_correct_set_chosen_2 %>% filter(trial_type == "large-DR") %>% pull(mean) %>% force_decimals()`, $SD$ = `r df.summary_correct_set_chosen_2 %>% filter(trial_type == "large-DR") %>% pull(sd) %>% force_decimals()`; `r print_ttest(t_largedr_2)`). In large-set trials with an off-by-one difference ($M$ = `r df.summary_correct_set_chosen_2 %>% filter(trial_type == "large-NR") %>% pull(mean) %>% force_decimals()`, $SD$ = `r df.summary_correct_set_chosen_2 %>% filter(trial_type == "large-NR") %>% pull(sd) %>% force_decimals()`), children performed at chance overall (`r print_ttest(t_largenr_2)`; Figure \@ref(fig:exp2-plot)A). Performance in this experiment was even more variable than in Experiment 1 (Figure \@ref(fig:exp2-plot-var)).

We next examined children’s performance specifically on the off-by-one trials (Large-NR), which can only be solved correctly using one-to-one correspondence. Out of `r df.ppt_2 %>% nrow()` children tested, only `r ppt_correct_nr_2 %>% length()` (`r force_decimals(ppt_correct_nr_2 %>% length() / df.ppt_2 %>% nrow() * 100)`%) succeeded in all trials, showing some evidence of using one-to-one correspondence in extending number words^[On average, these `r ppt_correct_nr_2 %>% length()` children were slightly older and had a greater Highest Count score ($M_{age}$ = `r df.ppt_correct_nr_2 %>% pull(mean_age) %>% force_decimals()`; $M_{HC}$ = `r df.ppt_correct_nr_2 %>% pull(mean_hc) %>% force_decimals()`) compared to children who did not succeed in all trials ($M_{age}$ = `r df.ppt_not_all_correct_nr_2 %>% pull(mean_age) %>% force_decimals()`; $M_{HC}$ = `r df.ppt_not_all_correct_nr_2 %>% pull(mean_hc) %>% force_decimals()`), though due to the small number of children we could not compare these values. Note that one of these `r ppt_correct_nr_2 %>% length()` children contributed only 1 large trial with an off-by-one difference because they began counting the bunnies before the animals disappeared on the other 2 trials.].

```{r exp2-plot, fig.cap = "Each dot represents a participant. The width of the shaded area of violin plots represents the proportion of the data located there. Error bars represent bootstrapped 95% CIs. A) Proportion of correct item set choice by participant against trial type. Horizontal dashed line indicates performance at chance = 0.50. B) Proportion of correct numerical response by participant against trial type.", fig.height = 4, include = T}
...
plot_summary_count_2 <- ggplot(df.summary_2,
       aes(x = mean_correct_count_when_correct_set_chosen)) +
  geom_line(stat = 'count') + 
  geom_point(stat = 'count', 
             size = 2) +
...
df.summary_trial_type_2 <- df.trial_2 %>%
  group_by(trial_type, id) %>%
  summarise(mean_correct_set_chosen = mean(correct_set_chosen),
            mean_correct_count_when_correct_set_chosen = mean(correct_count_when_correct_set_chosen))

plot_summary_set_trial_type_2 <- ggplot(data = 
                                            df.summary_trial_type_2 %>% 
                                            ungroup() %>%
         mutate(trial_type = factor(trial_type, labels = c("small sets (e.g., 1:2)", "large sets, discriminable ratio (e.g., 5:10)", "large sets, off-by-one (e.g., 9:10)"))),
       aes(x = mean_correct_set_chosen,
           color = trial_type, 
           shape = trial_type)) +
  geom_line(stat = 'count') + 
  ...
  labs(y = "Number of participants", 
       x = "Prop. of correct set choice",
       color = "Set Size and Ratio", 
       shape = "Set Size and Ratio")

legend <- get_legend(
  # create some space to the left of the legend
  plot_summary_set_trial_type_2 + 
    guides(color = guide_legend(nrow = 1)) 
)

plot_summary_count_trial_type_2 <- ggplot(data = 
                                            df.summary_trial_type_2 %>% 
                                            ungroup() %>%
         mutate(trial_type = factor(trial_type, labels = c("small sets (e.g., 1:2)", "large sets, discriminable ratio (e.g., 5:10)", "large sets, off-by-one (e.g., 9:10)"))),
       aes(x = mean_correct_count_when_correct_set_chosen, 
...
combined_plot <- plot_grid(
  plot_summary_set_2 + theme(legend.position = "none"),
  plot_summary_count_2 + theme(legend.position = "none"), 
  plot_summary_set_trial_type_2 + theme(legend.position = "none"),
  plot_summary_count_trial_type_2 + theme(legend.position = "none"), 
  nrow = 2, 
  ncol = 2,
  labels = c("A", "B", "C", "D")
)

plot_grid(combined_plot, legend, ncol = 1, 
          rel_heights = c(10, 2))
```
...
To further analyze the effect of trial type on selection of the Correct Set Choice, we constructed a GLMM with the formula Correct Set Choice (0/1) ~ Age (z-scored) + (1 | Participant) + (1 | Trial), and tested whether adding Trial Type (Small sets / Large sets with perceptually discriminable ratios (Large-DR) / Large sets with an off-by-one difference (Large-NR)) as fixed effects improved the fit of the model. All models included by-subject and by-item random intercepts. We found no effect of age in our base model (`r print_model_comp_p(fit.anova.base_set_2, "age_zscored")`). When trial type was included as a fixed effect, we found a significant effect of trial type (`r print_model_comp(fit.anova.trial_type_set_2, "trial_type")`), and still no effect of age (`r print_model_comp_p(fit.anova.trial_type_set_2, "age_zscored")`). The model with Trial Type explained significantly more variation in the observed data compared to the base model in a likelihood ratio test (`r print_model_comp(fit.compare.trial_type_set_2, "fit.trial_type_set_2")`, `r print_model_fit(fit.trial_type_set_2, fit.base_set_2)`). This finding replicated our finding from Experiment 1, which found that age did not explain cardinal extension performance in CP-knowers. *Post hoc* pairwise comparisons between the three trial types (Small, Large-DR, Large-NR) with Bonferroni correction found that children were more likely to provide correct responses in Small trials compared to Large-DR trials ($z$ = `r fit.contr.trial_type_set_2 %>% filter(contrast == "small - (large-DR)") %>% pull(z.ratio) %>% force_decimals()`, `r fit.contr.trial_type_set_2 %>% filter(contrast == "small - (large-DR)") %>% pull(p.value) %>% format_p()`) and Large-NR trials ($z$ = `r fit.contr.trial_type_set_2 %>% filter(contrast == "small - (large-NR)") %>% pull(z.ratio) %>% force_decimals()`, `r fit.contr.trial_type_set_2 %>% filter(contrast == "small - (large-NR)") %>% pull(p.value) %>% format_p()`). Success in Large-DR trials was also significantly higher than in Large-NR trials ($z$ = `r fit.contr.trial_type_set_2 %>% filter(contrast == "(large-DR) - (large-NR)") %>% pull(z.ratio) %>% force_decimals()`, `r fit.contr.trial_type_set_2 %>% filter(contrast == "(large-DR) - (large-NR)") %>% pull(p.value) %>% format_p()`).
...
df.error_obs <- df.trial_2 %>%
  filter(#id %in% ppt_succeed_in_small,
         trial_type != "small") %>%
...
apa_table(
  ...
  align = c("m{5.75cm}", rep("m{4.75cm}", 2)),
...
ppt_count_mistake_2 <- df.summary_count_2 %>%
  filter(mean_correct_count_when_correct_set_chosen < 1) %>%
  pull(id)

df.trial_count_wrong_2 <- df.trial_2 %>%
  filter(!is.na(final_count), 
         correct_count_when_correct_set_chosen == 0)
```
...
`r df.trial_count_wrong_2 %>% pull(id) %>% unique() %>% length() %>% printnum(., numerals = F, capitalize = T)` of the `r df.ppt_2 %>% nrow()` children tested (`r force_decimals(df.trial_count_wrong_2 %>% pull(id) %>% unique() %>% length() / df.ppt_2 %>% nrow() * 100)`%) gave a numerical response that was wrong on at least 1 trial, resulting in a total of `r df.trial_count_wrong_2 %>% nrow()` (out of `r df.trial_2 %>% nrow()`) trials with the wrong numerical response.
Out of these `r df.trial_count_wrong_2 %>% nrow()` trials, `r df.trial_count_wrong_2 %>% filter(correct_set_chosen == 1) %>% nrow()` trials (`r df.trial_count_wrong_2 %>% filter(correct_set_chosen == 1) %>% pull(id) %>% unique() %>% length()` children) could be attributed to procedural error, where participants chose the correct set, but then counted wrongly. 
On `r df.trial_count_wrong_2 %>% filter(correct_set_chosen == 0, !is.na(set_chosen)) %>% nrow()` trials (`r df.trial_count_wrong_2 %>% filter(correct_set_chosen == 0, !is.na(set_chosen)) %>% pull(id) %>% unique() %>% length()` children), participants counted the wrong set or both sets, which indicated failure in identifying the correct set for cardinal extension inference. 
Finally, on `r df.trial_count_wrong_2 %>% filter(is.na(set_chosen)) %>% nrow()` trials (`r df.trial_count_wrong_2 %>% filter(is.na(set_chosen)) %>% pull(id) %>% unique() %>% length()` children), participants provided the wrong response but did not point to either set, which can either reflect overconfidence in estimating the number of items in the correct item set, or a lack of conceptual understanding about how the item set in one-to-one correspondence with the bunnies can be used to infer the number of bunnies. In comparison to the error pattern of CP-knowers’ numerical response in Experiment 1, Experiment 2 found a larger proportion of errors that came from counting the wrong set or both sets, with this type of mistake accounting for more than half or the errors. 
...
check_collinearity(fit.trial_type_set_2_hc)
summary(fit.trial_type_set_2_hc)
...
fit.trial_type_count_2_hc <- glmer(correct_count_when_correct_set_chosen ~ scale(highest_count) + trial_type + age_zscored + (1|id) 
                                   #+ (1|trial_ratio)
                                   , 
                          data = df.trial_2 %>%
                            filter(!is.na(highest_count)), family="binomial")

check_collinearity(fit.trial_type_count_2_hc)


fit.compare.trial_type_count_2_hc <- anova(fit.trial_type_count_2_hc, fit.trial_type_count_2_hc_comp, type=3)
```

As an exploratory analysis, we added children’s Highest Count as a fixed effect to the best-performing models from the two analysis approaches above. `r df.ppt_2 %>% filter(!is.na(highest_count)) %>% nrow() %>% printnum(., numerals = F, capitalize = T)` children (out of `r df.ppt_2 %>% nrow()`) provided a Highest Count, and performed similarly to the CP-knowers from Experiment 1 ($M_{HC}$ = `r df.hc_summary_2 %>% pull(mean_hc) %>% force_decimals()` [`r df.hc_summary_2 %>% pull(min_hc)`; `r df.hc_summary_2 %>% pull(max_hc)`]; $SD_{HC}$ = `r df.hc_summary_2 %>% pull(sd_hc) %>% force_decimals()`; for the distribution of Highest Count by Correct Set Choice performance, see Figure \@ref(fig:exp2-hc-plot) and Figure S2 in supplemental material). After excluding data from the `r df.ppt_2 %>% filter(is.na(highest_count)) %>% nrow() %>% printnum(., numerals = F)` children who did not provide a Highest Count, we found the same pattern as in Experiment 1: Highest Count did not explain additional variability for either Correct Set (`r print_model_comp(fit.compare.trial_type_set_2_hc, "fit.trial_type_set_2_hc")`, `r print_model_fit(fit.trial_type_set_2_hc, fit.trial_type_set_2_hc_comp)`) or Correct Numerical Response (`r print_model_comp(fit.compare.trial_type_count_2_hc, "fit.trial_type_count_2_hc")`, `r print_model_fit(fit.trial_type_count_2_hc, fit.trial_type_count_2_hc_comp)`). Similar to Experiment 1, Highest Count showed low multicollinearity with other predictors [Variance Inflation Factor < 2, following the threshold established by @james2013].

```{r exp2-hc-plot, fig.cap = "Participants’ average Highest Count by performance in the Correct Set Choice measure.", fig.height = 4, include = T}
ggplot(df.trial_2 %>%
         filter(!is.na(highest_count)) %>%
  ...
  coord_cartesian(ylim=c(0,40)) +
  labs(y = "Average Highest Count", 
       x = "Number of trials with Correct Set Choice (out of 9)")
```
